define('File/utils/filter', [
    ...
    'File/LocalFile',
    'File/Directory'
], function (require, exports, ExtensionsError, MaxSizeError, LocalFile, Directory) {
    'use strict';
    var KB = 1024;
    var MB = KB * KB;
    return function (fileList, _a) {
        ...
        for (var i = 0; i < fileList.length; i++) {
            var item = fileList instanceof FileList ? fileList.item(i) : fileList[i];    // Если пришла уже ошибка из внутренней фильтрации ResourceGetter
            // Если пришла уже ошибка из внутренней фильтрации ResourceGetter
            if (item instanceof Error || item instanceof Directory) {
                results.push(item);
                continue;
            }
