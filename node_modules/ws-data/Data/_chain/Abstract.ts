...
import { Abstract as AbstractType } from '../type';
...
export default abstract class Abstract<T> extends AbstractType implements IEnumerable<T> /** @lends WS.Data/Chain/Abstract.prototype */{
   readonly '[Data/_collection/IEnumerable]' = true;
   ...
   /**
    * @property {WS.Data/Chain/Abstract|null>} Предыдущий элемент цепочки
    */
   ...
   map(callback: (item: any, index: number) => any, thisArg?: Object): Mapped<T> {
      const Next = di.resolve('Data/chain:Mapped');
      return new Next(
         this,
         callback,
         thisArg
    ...
    */
   zip(...args): Zipped<T> {
      const Next = di.resolve('Data/chain:Zipped');
      return new Next(
         this,
         args
      );
   ...
   concat(...args: Array<Array<T>|IEnumerable<T>>): Concatenated<T> {
      const Next = di.resolve('Data/chain:Concatenated');
    ...
    */
   flatten(): Flattened<T> {
      const Next = di.resolve('Data/chain:Flattened');
   ...
   group(key: string|((item: any) => string), value: string|((item: any) => any)): Grouped<T> {
      const Next = di.resolve('Data/chain:Grouped');
      return new Next(
         this,
         key,
         value
   ...
   count(by?: string|((item: any) => string)): Counted<T>|number {
      ...
      const Next = di.resolve('Data/chain:Counted');
   ...
   uniq(idExtractor?: (item: any) => string|number): Uniquely<T> {
      const Next = di.resolve('Data/chain:Uniquely');
   ...
   filter(callback: (item: any, index: number) => boolean, thisArg?: Object): Filtered<T> {
      const Next = di.resolve('Data/chain:Filtered');
      return new Next(
         this,
         callback,
         thisArg
   ...
   first(n?: number): Sliced<T> | any {
      ...
      const Next = di.resolve('Data/chain:Sliced');
    ...
    */
   reverse(): Reversed<T> {
      const Next = di.resolve('Data/chain:Reversed');
      return new Next(this);
   }
   ...
   sort(compareFunction: CompareFunction): Sorted<T> {
      const Next = di.resolve('Data/chain:Sorted');
...
// @ts-ignore
Abstract.prototype['[Data/_chain/Abstract]'] = true;
// @ts-ignore
