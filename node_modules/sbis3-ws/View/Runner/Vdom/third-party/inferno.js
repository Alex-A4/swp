define('View/Runner/Vdom/third-party/inferno',

    ['View/Runner/expressions/RawMarkupNode',
     'Core/helpers/String/unEscapeASCII',
     'Core/detection',
     'Core/IoC',
     'Core/Debug' //No mappings defined for ILogger
    ],
function (RawMarkupNode, unEscapeASCII, detection, IoC) {
    'use strict';
    ...
    function unescape(s) {
        ...
        //Безопасная функция. Используется для вставки строки в текстовую ноду. Все символы при такой вставке экранируются браузером.
        //То есть, если мы вставим node.nodeValue = "<script>"; - мы увидим строку с угловыми скобками и это не будет выполнено.
        var translate_re = /&(nbsp|amp|quot|apos|lt|gt);/g,
         translate = {"nbsp": String.fromCharCode(160),"amp" : "&","quot": "\"","apos": "'","lt"  : "<","gt"  : ">"};
        // unescape юникода
        s = unEscapeASCII(s);
    ...
    function duplicateKeys(key, foundKeys) {
        var splitKey,
            ...
            keys = Object.keys(foundKeys).filter(function (keyItem) {
        ...
        splitKey = keys.map(function (keyItem) {
            return parseInt(keyItem.split(duplicate)[1]);
        }).reduce(findMaxInArray);
        return duplicate + (parseInt(splitKey) + 1);
    }
    var exports = {};
    var NO_OP = '$NO_OP';
    ...
    function directClone(vNodeToClone) {
        ...
        } else if (flags === undefined && typeof vNodeToClone.markup === 'string' /* RawMarkupNode Type WS bugfix */) {
           newVNode = vNodeToClone;
        }
        return newVNode;
    }
    ...
    function mount(vNode, parentDom, context, isSVG, isRootStart) {
        var flags = vNode.flags;
        if (flags & 481 /* Element */) {
            return mountElement(vNode, parentDom, context, isSVG, isRootStart);
        }
        if (flags & 14 /* Component */) {
    ...
    function mountElement(vNode, parentDom, context, isSVG, isRootStart) {
        var flags = vNode.flags;
        var children = vNode.children;
        var props = vNode.props;
    ...
    function isIgnoredNode(nextSibling) {
        return  (nextSibling && nextSibling.tagName === 'SCRIPT' &&
           ...
           (nextSibling && nextSibling.attributes &&
    ...
    function hydrateElement(vNode, dom, context, isSVG) {
        ...
        if (dom.nodeType !== 1 || dom.tagName.toLowerCase() !== vNode.type) {
            ...
            childNode = isIgnoredNode(childNode) ? skipIgnoredNode(childNode) : childNode;
            var childFlags = vNode.childFlags;
            if ((childFlags & 1 /* HasInvalidChildren */) === 0) {
                ...
                if (childFlags === 2 /* HasVNodeChildren */) {
                    ...
                    for (var i = 0, len = children.length; i < len; i++) {
                        ...
                        if (isNull(childNode)) {
                            ...
                            nextSibling = skipIgnoredNode(childNode);
                            if (!isIgnoredNode(childNode)) {
            ...
            else if (!isNull(dom.firstChild) && !isSamePropsInnerHTML(dom, props) && !hasOnlyIgnoredChildren(dom)) {
        ...
        if (lastVNode.type !== nextTag) {
            ...
            if (!dom && parentDom) {
               ...
               for(var k=0; k<elements.length; k++) {
                   if (elements[k].attributes.key && elements[k].attributes.key.value === lastVNode.key){
    ...
    function render(input, parentDom, callback, isRootStart) {
        // Development warning
        {
        ...
        if (isNullOrUndef(rootInput)) {
            if (!isInvalid(input)) {
                ...
                if (isNull(parentDom.firstChild)) {
                    mount(input, parentDom, EMPTY_OBJ, false, isRootStart);
                    parentDom.$V = input;
                }
                else {
                    hydrate(input, parentDom, callback, isRootStart);
                }
                rootInput = input;
            }
        }
