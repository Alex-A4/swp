...
function IOSVersion(userAgent) {
   const match = /\bCPU\s+(i?)OS\s+(\d+)/.exec(userAgent);
   return match && parseInt(match[2], 10);
}
...
function getWebkitVersion(userAgent) {
   const match = /(AppleWebKit\/)+([\w.]+)/.exec(userAgent);
   return match && parseInt(match[2], 10);
}
...
function getSafariVersion(userAgent) {
   const match = /(Safari\/)+([\w.]+)/.exec(userAgent);
   return match && parseInt(match[2], 10);
}
...
function getUserAgent() {
   ...
   } else if (typeof navigator !== 'undefined') { // browser
...
function getTrident(userAgent) {
   const match = userAgent.match(/Trident\/(\d+)/);
...
function recalcDetection() {
   const
       ...
       */
      ieVersion = IEVersion(userAgent),
      iosVersion = IOSVersion(userAgent),
      ...
      isWPMobilePlatform = /Windows Phone/i.test(userAgent) || /iemobile/i.test(userAgent) || /WPDesktop/i.test(userAgent) || /Lumia/i.test(userAgent),
      isMacOSPlatform = !!(/\bMac\s+OS\s+X\b/.test(userAgent)) && !isIOSMobilePlatform,
      isChromeIOS = !!(isIOSMobilePlatform && userAgent.match(/\bCriOS\b/)),
      isChromeDesktop = (!!(typeof window !== 'undefined' && window.chrome) || isChromeIOS) && !ieVersion,
      isNotFullGridSupport = !!(((userAgent.match(/Chrome\/[0-9]*./) !== null) && (userAgent.match(/Chrome\/[0-9]*./)[0].substr(7, 8) < 65)) || ((userAgent.match(/Version\/[0-9]*.[0-9].[0-9] Safari/) !== null) && (userAgent.match(/Version\/[0-9]*.[0-9].[0-9] Safari/)[0].substr(11, 1) < 1) && (userAgent.match(/Version\/[0-9]*.[0-9].[0-9] Safari/)[0].substr(8, 2) !== '12'))),
      isMobilePlatform = isIOSMobilePlatform || isAndroidMobilePlatform || isWPMobilePlatform,
      isMobileSafari = isIOSMobilePlatform && !isChromeIOS && /AppleWebKit/.test(userAgent) && /Mobile\//.test(userAgent),
      isWebkit = !isChromeDesktop && /(webkit)/i.test(userAgent) && !(ieVersion && ieVersion >= 12),
      isDesktopSafari = isMacOSPlatform && /AppleWebKit/.test(userAgent) && !/Chrome/.test(userAgent),
      isSafari = isMobileSafari || isDesktopSafari,
      isOldWebkit = (isChromeIOS && iosVersion < 11) || (isSafari && getSafariVersion(userAgent) < 604),
      win = /Windows/i.test(userAgent),
      win10 = /Windows NT 10\.0/i.test(userAgent),
      win8 = /Windows NT 6\.[23]/i.test(userAgent),
      win7 = /Windows NT 6\.1/i.test(userAgent),
      winVista = /Windows NT 6\.0/i.test(userAgent),
      winXP = /Windows NT 5\.[12]/i.test(userAgent),
      unix = /(?:unix|linux)/i.test(userAgent),
      mac = /Macintosh/i.test(userAgent);
   let safariVersion = isSafari && userAgent.match(/Version\/([0-9\.]*)/);
