...
_bigCalc = function() {
   {
      ...
      function BigInteger(n, s) {
         ...
         // без этого в iPad'е строка "this._d = n;" по непонятным причинам не срабатывает: this._d остается равным undefined
         this._s = this._d = undefined;
                  ...
                  defClass: function(name, opts) {
                        ...
                        (bctor && !(ctor.prototype instanceof bctor)))
                     {
                        ...
                        if (ctor.prototype) {
                           ...
                           for (key in ctor.prototype) {
                            ...
                            //фикс: IE8 не видит этих свойств в цикле
                           proto.valueOf = ctor.prototype.valueOf;
                           proto.toString = ctor.prototype.toString;
                        }
                     ...
                     if (typeof base !== "undefined") {
                        ...
                        for (cname in classes) {
                            if(!_hasOwnProperty(classes, cname))
                               continue;
   ...
   function parseExpression(str, args) {
      ...
      function convertExp(str) {
         return str.replace(/([0-9]+(?:\.[0-9]+)?)(?:e(-|\+)?([0-9]+))?/gi, function(s, n, minus, exp) {
            var expVal = '';
            if (exp) {
               exp = parseInt(exp, 10);
               expVal = '1';
...
bigCalc = (function() {
    var initialized;
    function init() {
        initialized = initialized ? initialized : _bigCalc();
        return initialized.apply(this, arguments);
    }
    return init;
})();
...
if (typeof define == 'function') {
   /**
    * Сторонняя библиотека для работы с большими числами.
    * Библиотека позволяет произвести несколько основных арифметических операций, включая добавление, вычитание, умножение и деление, которые, при задании только точных аргументов, должны возвращать точный, численно правильный результат.
    * @class WS/ext/bignumWrapper
    * @public
    * @example
    * <pre>
    *  require(['WS/ext/bignumWrapper'], function(BigNum) {
    *
    *     // Значение по умолчанию - 0.
    *     var a = new BigNum();
    *
    *     // При добавлении числа к исходному значению получаем - 123456.
    *     a = a.add(123456);
    *
    *     // При сложении двух одинаковых значений a получаем - 246912.
    *     a = a.add(a);
    *
    *     // При делении аргумента на значение a получаем - ~ 35273.1428571429.
    *     a = a.divide(7);
    *
    *     // Возвращает "35273.14".
    *     a + '';
    *
    *     // При сложении значения a с аргументом получаем - ~ 35273.5428571429.
    *     a = a.add('4/10');
    *
    *     // Возвращаем полученный результат в виде строки без знаков после запятой - "35274".
    *     a.toString(0);
    *
    *     a = a.divide(a.multiply(a.sub(1)));
    *
    *     // Возвращаем дробную часть с десятью знаками после запятой - "0.0000283507".
    *     a.toString(10);
    *  });
    * </pre>
    */
   define('WS/ext/bignumWrapper', [], function() {
      return bigCalc;
   });
}