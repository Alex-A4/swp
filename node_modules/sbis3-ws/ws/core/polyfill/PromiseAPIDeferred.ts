...
function addCallback(callback) {
    let def = new Deferred().addCallback(callback);
    this.then(res => {
...
function addErrback(onerror) {
    let def = new Deferred().addErrback(onerror)
...
function addCallbacks(callback, errback) {
    let def = new Deferred().addCallbacks(callback, errback);
    this.then(res => {
        if (res instanceof Error) {
            def.errback(res);
            return;
...
function addBoth(callback) {
    let def = new Deferred().addBoth(callback);
    this.then(res => {
        if (res instanceof Error) {
            def.errback(res);
            return;
 ...
 */
function dependOn(master): Promise<any> | void {
    if (master instanceof Promise) {
        throw new Error('Нельзя вызвать метод dependOn у Promise');
    }
    let callback = (value) => value;
    let errback = (error) => { throw error };

    let promise = new Promise((resolve: (error: any) => never, reject: (error: any) => never) => {
        callback = resolve;
        errback = reject;
    });
